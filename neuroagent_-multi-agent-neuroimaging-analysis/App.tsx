...
const App: React.FC = () => {
  ...
  // Track workflow start time for elapsed calculation on finalize
  ...
  useEffect(() => {
    const initSystem = async () => {
      ...
      if (isOllamaUp) {
        ...
        addMessage(AgentType.SYSTEM, "CRITICAL WARNING: Could not connect to Ollama.");
      }
      try {
        ...
        addMessage(AgentType.SYSTEM, "Warning: Could not connect to MCP Server. Using internal tools.");
      }
    };
    initSystem();
  ...
  // We need a ref to messages for use inside async functions
  const messagesRef = useRef(messages);
  messagesRef.current = messages;

  const addMessage = (role: AgentType, content: string, metadata?: any): ChatMessage => {
    let usedModel: string | undefined;
    if (role === AgentType.ORCHESTRATOR || role === AgentType.GENERAL_PLANNER) usedModel = selectedGeneralModel;
    else if (role === AgentType.NEURO_PLANNER || role === AgentType.PLAN_VALIDATOR || role === AgentType.PREPROCESSOR || role === AgentType.RESEARCHER) usedModel = selectedNeuroModel;
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = (e) => { loadData(e.target?.result as string, file.name); };
    reader.readAsText(file);
  };

  const handleLoadDemo = () => loadData(MOCK_CSV_DATA, "Amyloid_SUVR_Swapped.csv");

  const handleGeneralModelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const m = e.target.value; setSelectedGeneralModel(m); setGeneralModel(m);
    addMessage(AgentType.SYSTEM, `General Model switched to: ${m}`);
  };
  const handleNeuroModelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const m = e.target.value; setSelectedNeuroModel(m); setNeuroModel(m);
    addMessage(AgentType.SYSTEM, `Neuro Model switched to: ${m}`);
  };
  ...
  const loadData = (csvText: string, name: string) => {
    ...
    setVisualizations([{ type: VisualizationType.DATA_TABLE, title: 'Data Inspection', data }]);
  };
  ...
  const finalizeWorkflow = (phase: 'done' | 'error') => {
    ...
    // Use a short timeout to ensure last setMessages has flushed
    setTimeout(() => {
  ...
  const executePlanSteps = async (
    plan: any, startStepIndex: number = 0, initialParamsOverride: any = null,
    currentData: any[], currentColumns: string[], intent: 'RESEARCH' | 'GENERAL'
  ) => {
    ...
    for (let i = 0; i < stepsToRun.length; i++) {
      ...
      const executorMsg = addMessage(AgentType.EXECUTOR, `Executing Step ${step.step_id}: ${step.tool}...`,
        { plan, stepIndex: startStepIndex + i, tool: step.tool, params });
      stepIdToMessageId[step.step_id] = executorMsg.id;
      let stepResult = ""; let viz: ToolVisualization | null = null;
      ...
      try {
        if (internalToolDef) {
          if (step.tool === 'TRANSFORM_DATA') {
            ...
            if (currentColumns.includes(col)) {
              ...
              const mapping = mappingResult.mapping; const rationale = mappingResult.rationale;
              setMessages(prev => prev.map(m => m.id === preMsg.id ? { ...m, content: `**Analysis of '${col}':** ${rationale || 'Mapping generated.'}\n\nMapping: ${JSON.stringify(mapping)}` } : m));
              const result = executeInternalTool(step.tool, { ...params, mapping }, currentData);
              const tr = result as any; currentData = tr.transformedData; const newColName = tr.newColumn;
              ...
              viz = { type: VisualizationType.DATA_TABLE, title: `Preprocessing: ${col} -> ${newColName}`, data: Object.entries(mapping).map(([k,v]) => ({ Original: k, Numeric: v })) };
            } else { stepResult = `Error: Column '${col}' not found.`; }
          } else if (step.tool === 'MODIFY_VISUALIZATION') {
            const result = executeInternalTool(step.tool, params, currentData);
            setVisualizations(prev => { if (prev.length === 0) return prev; const ti = prev.findIndex(v => v.messageId === highlightedMessageId); const idx = ti !== -1 ? ti : 0; const u = [...prev]; const t = { ...u[idx] }; t.config = { ...t.config, ...result }; if (result.title) t.title = result.title; u[idx] = t; return u; });
            ...
            if (step.tool === 'CORRELATION_ANALYSIS') {
              ...
              viz = { type: VisualizationType.SCATTER_PLOT, title: `Correlation: ${cr.r.toFixed(2)}`, data: { ...cr, xCol: params.x_column || params.x || 'X', yCol: params.y_column || params.y || 'Y' } };
              stepResult = `Correlation Analysis complete. R=${cr.r.toFixed(3)}, p-value=${cr.p.toExponential(3)}.`;
            } else if (step.tool === 'GROUP_COMPARISON') {
              const gr = result as any;
              viz = { type: VisualizationType.BOX_PLOT, title: 'Group Comparison', data: gr };
              stepResult = `Group Comparison complete. ANOVA p-value=${gr.pVal.toExponential(3)}.`;
            }
          }
        } else if (mcpToolDef) {
          const args = { ...params }; if (mcpToolDef.inputSchema.properties && 'data' in mcpToolDef.inputSchema.properties) args.data = currentData;
          const result = await mcpClient.callTool(step.tool, args);
          const tc = result.content.filter(c => c.type === 'text').map(c => c.text).join('\n');
          stepResult = tc || "Tool executed successfully."; viz = parseMcpResultToVisualization(step.tool, tc);
        ...
        } else { stepResult = `Unknown tool: ${step.tool}. Skipping.`; }
      } catch (e: any) { stepResult = `Error: ${e.message}`; }

      setMessages(prev => prev.map(m => m.id === executorMsg.id ? { ...m, content: `${m.content}\n\n\u2705 ${stepResult}` } : m));
      if (viz) { viz.messageId = executorMsg.id; addVisualization(viz); }
    ...
    if (intent === 'RESEARCH') {
      ...
      setMessages(prev => prev.map(m => m.id === researchMsg.id ? { ...m, content: finalInsights } : m));
      addVisualization({ type: VisualizationType.RESEARCH_REPORT, title: "Scientific Research Report", data: { report: finalInsights, stepIdToMessageId }, messageId: researchMsg.id });
    } else {
  ...
  const handleRestartFromStep = async (messageId: string, newParams: any) => {
    ...
    // Find which workflow this message belongs to, truncate everything after it
    // This is the "branch restart" behavior like ChatGPT's edit
    const truncateIndex = msg.role === AgentType.EXECUTOR ? msgIndex : msgIndex + 1;
    
    // Truncate messages and visualizations
    const newMessages = messages.slice(0, truncateIndex);
    ...
    // Find and remove all workflows that start at or after this point
    for (let i = history.records.length - 1; i >= 0; i--) {
      const rec = history.records[i];
      if (rec.startIndex >= truncateIndex) {
        wf.truncateAfter(rec.id);
      } else if (truncateIndex > rec.startIndex) {
        // This workflow contains the edit point — truncate from here
        wf.truncateAfter(rec.id);
        break;
      }
    }
    ...
    if (msg.role === AgentType.EXECUTOR) {
      if (!msg.metadata || msg.metadata.stepIndex === undefined) { setIsProcessing(false); return; }
      const { plan, stepIndex } = msg.metadata;
      
      // Start a new workflow for the restart
      wf.startNewQuery(`Restart: Step ${stepIndex + 1}`, newMessages.length);
      ...
      wf.startNewQuery(`Re-plan: ${msg.metadata?.plan?.rationale?.slice(0, 40) || 'Manual edit'}`, newMessages.length);
  ...
  const handleUserQuery = async (query: string) => {
    ...
    setIsProcessing(true);
    workflowStartRef.current = Date.now();
    wf.startNewQuery(query, messages.length + 1); // +1 because the User message was just added

    try {
      if (!ollamaConnected) {
        const recheck = await checkOllamaConnection();
        if (!recheck) { addMessage(AgentType.SYSTEM, "Error: Ollama is still unreachable."); finalizeWorkflow('error'); setIsProcessing(false); return; }
      ...
      let plan: any = { analysis_steps: [] }; let planIsValid = false;
      let planningRetries = 0; const MAX_PLANNING_RETRIES = 3; let currentFeedback = "";
      ...
      while (!planIsValid && planningRetries < MAX_PLANNING_RETRIES) {
        ...
        if (validation.valid) {
          ...
          if (planningRetries >= MAX_PLANNING_RETRIES) {
            addMessage(AgentType.SYSTEM, "Critical: Planning failed after multiple validation cycles.");
            finalizeWorkflow('error');
            setIsProcessing(false);
            return;
          }
  ...
  const handleVizClick = (messageId?: string) => { if (messageId) setHighlightedMessageId(messageId); };
    ...
    <div className="flex h-screen w-full overflow-hidden bg-slate-950 text-slate-200">
      <div className="w-1/2 p-4 flex flex-col h-full border-r border-slate-800">
          ...
          {availableModels.length > 0 && (
            <div className="flex gap-2 text-xs">
              <div className="flex flex-col gap-1 w-1/2">
                ...
                <select value={selectedGeneralModel} onChange={handleGeneralModelChange} className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-300 focus:outline-none focus:border-indigo-500">
                  {availableModels.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1 w-1/2">
                <label className="text-slate-500">Neuro Model</label>
                <select value={selectedNeuroModel} onChange={handleNeuroModelChange} className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-300 focus:outline-none focus:border-indigo-500">
                  {availableModels.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
            </div>
        ...
        <div className="flex-1 min-h-0">
          <VisualizerArea visualizations={visualizations} datasetName={dataset?.name} onVizClick={handleVizClick} />
        </div>
      </div>
